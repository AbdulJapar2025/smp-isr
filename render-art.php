...
// Ambil input kategori dari user (manual input)
$filter = isset($_GET['cat']) ? trim($_GET['cat']) : '';

$sql    = "SELECT * FROM render_art WHERE status = 'active'";
$params = [];

// Filter berdasarkan kategori yang diinput (case insensitive)
if (!empty($filter)) {
...
<section class="py-16 bg-gradient-to-br from-purple-50 via-white to-indigo-50">
  <div class="max-w-7xl mx-auto px-4">
    <div class="text-center mb-12">
      <h1 class="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Event Sekolah</h1>
      <p class="text-lg text-gray-600 max-w-3xl mx-auto">
       Rayakan Kreativitas, Semangat, dan Prestasi!
      </p>
    </div>

    <!-- Filter Kategori dengan Input Manual -->
    <div class="flex flex-wrap justify-center items-center gap-3 mb-12">
      <form method="GET" action="<?= url('render-art.php') ?>" class="flex flex-wrap items-center gap-3">
        <div class="flex items-center gap-2">
          <label for="category-input" class="text-gray-700 font-medium">Kategori:</label>
          <input 
            type="text" 
            id="category-input"
            name="cat" 
            value="<?= h($filter) ?>"
            placeholder="Contoh: eksterior, interior, 3d_modeling"
            class="px-4 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent min-w-[300px]">
        </div>
        <button 
          type="submit"
        ...
        <?php if (!empty($filter)): ?>
          <a 
            href="<?= url('render-art.php') ?>"
            class="px-6 py-2 bg-gray-200 text-gray-700 rounded-full hover:bg-gray-300 transition font-medium">
            Reset
          </a>
        <?php endif; ?>
      </form>
    ...
    <!-- Kategori yang sedang difilter -->
    <?php if (!empty($filter)): ?>
      <div class="text-center mb-8">
        <p class="text-gray-600">
          Menampilkan karya dengan kategori: 
          <span class="font-semibold text-indigo-600"><?= h($filter) ?></span>
          <span class="text-gray-500">(<?= count($render_items) ?> hasil)</span>
        </p>
      </div>
    <?php endif; ?>
      ...
      <?php if (empty($render_items)): ?>
        ...
        <div class="col-span-3 text-center text-gray-500 py-12">
          ...
          <p class="text-xl font-medium">
            <?php if (!empty($filter)): ?>
              Tidak ada karya dengan kategori "<?= h($filter) ?>".
            <?php else: ?>
              Belum ada karya render yang tersedia.
            <?php endif; ?>
          </p>
